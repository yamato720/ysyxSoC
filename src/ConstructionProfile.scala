...
import _root_.scpu.{ConstructionProfile, FpgaConstructionConfig, HostConstructionConfig}
...
object DescribeConfig extends App {
  ...
  val metadata: HostConstructionConfig = construction
  ...
  val extra = entry.board.toSeq.flatMap { _ =>
    ...
    val fpga = construction match {
      case value: FpgaConstructionConfig => value
